...
This repository publishes a site, so a few rules are stricter than in the
code repositories:

- **Claims must be checkable.** If a page asserts that a contract, a rule or
  a decision behaves a certain way, link the file, the test or the transaction
  that shows it. Prose that cannot be checked does not belong here.
- **Do not hand-write what can be generated.** The golden-parity suite in
  `tests/engine.test.mjs` and the generated error registry exist so that
  documentation cannot silently drift from source. Extend them rather than
  restating their output by hand.
- **Fixtures are copies, not edits.** `data/` holds verbatim copies of
  upstream fixtures. If an upstream fixture changes, copy it again and update
  `data/PROVENANCE.json`. Editing a copy breaks the parity argument.
- **Never commit a secret.** The site is public. `scripts/check-links.py`
  fails the build on anything that looks like an admin key, a Stellar seed or
  a private key, but it is a backstop, not a licence to be careless.
